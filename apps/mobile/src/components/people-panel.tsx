...
export function PeoplePanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const contentPlatformStyle = Platform.select({
    ...
    web: {
      paddingTop: embedded ? Spacing.two : Spacing.six,
  ...
  async function handleDelete(id: string, name: string) {
    Alert.alert(
      t("people.removeTitle"),
      t("people.removeDescription", { name }),
      [
              ...
              <>
                {stats && (
                  <ThemedView style={styles.statsGrid}>
                    ...
                    />
                    <StatCard
                      label={t("people.stats.men")}
                      value={stats.men}
                      theme={theme}
                    />
                    <StatCard
                      label={t("people.stats.women")}
                      value={stats.women}
                      theme={theme}
                    />
                    <StatCard
                      label={t("people.stats.servicePrivilege")}
                      value={stats.servicePrivilege}
                      theme={theme}
                    />
                  </ThemedView>
                )}
                        ...
                        <ThemedView style={{ flex: 1 }}>
                          ...
                          <ThemedText type="small" themeColor="textSecondary">
                            ...
                            {person.privilegioServico
                              ? ` · ${t("people.servicePrivilegeLabel")}`
                              : ""}
...
function PersonForm({
  ...
  return (
    <ThemedView type="backgroundElement" style={styles.formCard}>
      <ThemedText
        type="default"
        style={{ fontWeight: "700", marginBottom: Spacing.three }}
      >
          ...
          {(["MALE", "FEMALE"] as const).map((sex) => (
            ...
            >
              <ThemedText
                type="small"
                style={[
                  form.sex === sex && { color: theme.primaryForeground },
                  { textAlign: "center" },
                ]}
              >
      ...
      <ThemedView style={styles.togglesSection}>
        ...
        <ToggleRow
          ...
          value={form.young}
          onValueChange={(v) => setForm({ ...form, young: v })}
          theme={theme}
        />
        <ToggleRow
          label={t("people.baptized")}
          value={form.batizado}
          onValueChange={(v) => setForm({ ...form, batizado: v })}
          theme={theme}
        />
        <ToggleRow
          label={t("people.cleaning")}
          value={form.limpeza}
          onValueChange={(v) => setForm({ ...form, limpeza: v })}
          theme={theme}
        />
        <ToggleRow
          label={t("people.student")}
          value={form.estudante}
          onValueChange={(v) => setForm({ ...form, estudante: v })}
          theme={theme}
        />
        {showPrivServico && (
          <ToggleRow
            label={t("people.servicePrivilegeLabel")}
            value={form.privilegioServico}
            onValueChange={(v) => setForm({ ...form, privilegioServico: v })}
            theme={theme}
          />
        )}
      </ThemedView>
      ...
      <ThemedView style={styles.togglesSection}>
        ...
        >
          {t("people.sectionFamily")}
        </ThemedText>
        <ToggleRow
          label={t("people.chefeFamilia")}
          value={form.chefeFamilia}
          onValueChange={(v) =>
            setForm({ ...form, chefeFamilia: v, familyId: "", familyName: "" })
          }
          theme={theme}
        />
        <ToggleRow
          label={t("people.casada")}
          value={form.casada}
          onValueChange={(v) => setForm({ ...form, casada: v })}
          theme={theme}
        />
        {form.chefeFamilia ? (
          <ThemedView style={styles.field}>
          ...
          <ThemedView style={styles.field}>
            ...
            <View style={styles.familyPicker}>
              ...
              >
                <ThemedText
                  type="small"
                  style={[!form.familyId && { color: theme.primaryForeground }]}
                >
                  {t("people.noFamily")}
                </ThemedText>
              </Pressable>
              {families.map((f) => (
                ...
                >
                  <ThemedText
                    type="small"
                    style={[
                      form.familyId === f.id && { color: theme.primaryForeground },
                    ]}
                  >
                    {f.name}
                  </ThemedText>
      ...
      {showDesig && (
        <ThemedView style={styles.togglesSection}>
          <ThemedText
            ...
            style={{ fontWeight: "600", marginBottom: Spacing.one }}
          >
            {t("people.sectionAssignments")}
          </ThemedText>
          <ToggleRow
            label={t("people.leituraBiblia")}
            value={
              ((form as Record<string, unknown>).leituraBiblia as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, leituraBiblia: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.microfoneVolante")}
            value={
              ((form as Record<string, unknown>).microfoneVolante as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, microfoneVolante: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.som")}
            value={((form as Record<string, unknown>).som as boolean) ?? false}
            onValueChange={(v) => setForm({ ...form, som: v } as typeof form)}
            theme={theme}
          />
          <ToggleRow
            label={t("people.video")}
            value={((form as Record<string, unknown>).video as boolean) ?? false}
            onValueChange={(v) => setForm({ ...form, video: v } as typeof form)}
            theme={theme}
          />
          <ToggleRow
            label={t("people.palco")}
            value={((form as Record<string, unknown>).palco as boolean) ?? false}
            onValueChange={(v) => setForm({ ...form, palco: v } as typeof form)}
            theme={theme}
          />
        </ThemedView>
      )}
      ...
      {showPriv && (
        <ThemedView style={styles.togglesSection}>
          <ThemedText
            ...
            style={{ fontWeight: "600", marginBottom: Spacing.one }}
          >
            {t("people.sectionPrivileges")}
          </ThemedText>
          <ToggleRow
            label={t("people.leitorEstudoBiblico")}
            value={
              ((form as Record<string, unknown>).leitorEstudoBiblico as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, leitorEstudoBiblico: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.leitorSentinela")}
            value={
              ((form as Record<string, unknown>).leitorSentinela as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, leitorSentinela: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.indicador")}
            value={
              ((form as Record<string, unknown>).indicador as boolean) ?? false
            }
            onValueChange={(v) =>
              setForm({ ...form, indicador: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.oracao")}
            value={
              ((form as Record<string, unknown>).oracao as boolean) ?? false
            }
            onValueChange={(v) =>
              setForm({ ...form, oracao: v } as typeof form)
            }
            theme={theme}
          />
        </ThemedView>
      ...
      {showPrivServicoSection && (
        <ThemedView style={styles.togglesSection}>
          <ThemedText
            ...
            style={{ fontWeight: "600", marginBottom: Spacing.one }}
          >
            {t("people.sectionServicePrivileges")}
          </ThemedText>
          <ToggleRow
            label={t("people.anciao")}
            value={((form as Record<string, unknown>).anciao as boolean) ?? false}
            onValueChange={(v) => setForm({ ...form, anciao: v } as typeof form)}
            theme={theme}
          />
          <ToggleRow
            label={t("people.presidenteVidaMinisterio")}
            value={
              ((form as Record<string, unknown>).presidenteVidaMinisterio as
                boolean) ?? false
            }
            onValueChange={(v) =>
              setForm({ ...form, presidenteVidaMinisterio: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.discursoTesouros")}
            value={
              ((form as Record<string, unknown>).discursoTesouros as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, discursoTesouros: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.joiasEspirituais")}
            value={
              ((form as Record<string, unknown>).joiasEspirituais as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, joiasEspirituais: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.nossaVidaCrista")}
            value={
              ((form as Record<string, unknown>).nossaVidaCrista as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, nossaVidaCrista: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.necessidadesLocais")}
            value={
              ((form as Record<string, unknown>).necessidadesLocais as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, necessidadesLocais: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.condutorEstudoBiblico")}
            value={
              ((form as Record<string, unknown>).condutorEstudoBiblico as
                boolean) ?? false
            }
            onValueChange={(v) =>
              setForm({ ...form, condutorEstudoBiblico: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.presidenteFimSemana")}
            value={
              ((form as Record<string, unknown>).presidenteFimSemana as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, presidenteFimSemana: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.discursoPublico")}
            value={
              ((form as Record<string, unknown>).discursoPublico as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, discursoPublico: v } as typeof form)
            }
            theme={theme}
          />
          <ToggleRow
            label={t("people.condutorSentinela")}
            value={
              ((form as Record<string, unknown>).condutorSentinela as boolean) ??
              false
            }
            onValueChange={(v) =>
              setForm({ ...form, condutorSentinela: v } as typeof form)
            }
            theme={theme}
          />
        </ThemedView>
