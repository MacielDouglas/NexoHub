...
- **Direção escolhida:** mundo bancário Itaú atual — tinta `#0c0c12` + laranja `#ec7000`, agora **global** (tokens em `:root`), com a dashboard clara isolada por `.light-world`. Hero "saldo" com total + 5 células de extrato, busca pill, linhas por família.
- **Decisões em aberto:** validação visual aguardando revisão do usuário no browser.